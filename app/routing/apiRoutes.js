    ...
    app.post('/api/friends', function(req, res) {
        ...
        if (friendsData.length > 1) {
            ...
            friendsData.forEach(function(user) {
                ...
                console.log("dif: " + totalDifference);
            ...
            // For each item in differences, if it is equal to the minimumDifference, add the corresponding friendsData to the bestMatches array.
            for (var i = 0; i < differences.length; i++) {
                if (differences[i] === minimumDifference) {
                    bestMatches.push(friendsData[i]);
                    console.log("best match:" + friendsData[i].name);
                    res.json(friendsData[i].name);
                }
            }
        ...
        // Adding the new user to the potential friends data.
        friendsData.push(thisUser);
    });
};